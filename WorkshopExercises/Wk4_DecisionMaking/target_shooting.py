...
def fire(x_coord, y_coord):
    # Display the bullet hole
    goto(x_coord, y_coord)
    dot(10)
    # Decide what score is appropriate
    if distance(0, 0) <= bullseye_diameter // 2:
        score = 10
    elif distance(0, 0) <= target_diameter // 2:
        score = 5
    else:
        score = 1
    # Write the score to the right of the bullet hole
    write('    ' + str(score), font=('Arial', 12, 'normal'))
    # Update the total
    global total_score
    total_score += score
    # Display the total
    title('Your total: ' + str(total_score) + ' points')
