...
print('Checking', html_file, 'for HTML tags')
...
document = html_code.find('<!DOCTYPE html>')
print('<!DOCTYPE html> tag found in position', document)
if document == -1:
   print("Tag couldn't be found")

# Put code here to confirm that the document includes a header section
# marked up as '<head> ... </head>'
document = html_code.find('<head>')
print('<head> tag found in position', document)
if document == -1:
   print("Tag couldn't be found")
...
document = html_code.find('<title>')
print('<title> tag found in position', document)
if document == -1:
   print("Tag couldn't be found")
...
document = html_code.find('<body>')
print('<body> tag found in position', document)
if document == -1:
   print("Tag couldn't be found")
