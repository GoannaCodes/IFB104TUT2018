...
#if there are still places where Weena appears:
while position != -1:
   #print the position where Weena appears
   print("Weena's name appears at position", position)
   #find next occuence where Weena appears
...
# returned by findall.
time_machine_text = open('TheTimeMachine.txt', 'U').read() # read the file's contents
#find all occurences where Weena appears
