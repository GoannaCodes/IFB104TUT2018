...
for row in range(grid_size): #repeat entire code 9 times (for each row)
    for number in range(grid_size): #choose number 9 times
        number = choice(num) #choose a random digit from string
        print(number, end = ' ')
    print()
##if number = choice(num) left as just choice(num) for if statement, choice function would just choose 0 - 9 sequentially
#however, if defined by a variable, choice function randomly selects number
